...
import { Callout, EmptyState, ErrorState, Page, PageHeader, Section, SourceLink } from "../design/primitives";
...
export function Rfps() {
  ...
  const rows = [...state.data].sort((a, b) => (a.deadline ?? "9999").localeCompare(b.deadline ?? "9999"));
    ...
    <Page>
      ...
      <Section>
        <Callout tone="info" title="Read a short list carefully">
          ...
          publish, and most of this category is invitation-only. A quiet board is not a quiet market.
        </Callout>
      </Section>
      ...
      <Section title={`Open (${open.length})`}>
        {open.length === 0 ? (
          ...
          <DataTable head={["Deadline", "Issuer", "Title", "Market", "Segment", "Status", "Source"]}>
            {open.map((r) => (
              <Tr key={r.slug}>
                <Td className="tnum whitespace-nowrap">{r.deadline}</Td>
                <Td>{r.issuer}</Td>
                <Td>{r.title}</Td>
                <Td><MarketTag country={r.country} /></Td>
                <Td><SegmentTag segment={r.segment} /></Td>
      ...
      {closed.length > 0 ? (
        <Section title={`Closed or undated (${closed.length})`} description="Kept so we do not re-find them.">
